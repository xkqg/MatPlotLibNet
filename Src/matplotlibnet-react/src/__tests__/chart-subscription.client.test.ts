...
describe('ChartSubscriptionClient', () => {
  ...
  it('invokes onChartUpdated callback when JSON update is received', async () => {
    const handler = vi.fn();
    const client = new ChartSubscriptionClient();
    client.onChartUpdated(handler);
    await client.connect('/charts-hub');

    const jsonCallback = mockOn.mock.calls.find((c: string[]) => c[0] === 'UpdateChart')![1];
    jsonCallback('chart-2', '{"title":"Live"}');

    expect(handler).toHaveBeenCalledWith('chart-2', '{"title":"Live"}');
  });
  ...
  it('is safe to dispose without connecting', async () => {
    const client = new ChartSubscriptionClient();
