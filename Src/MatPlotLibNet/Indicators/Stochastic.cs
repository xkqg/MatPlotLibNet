...
public sealed class Stochastic : CandleIndicator<StochasticResult>
{
    ...
    public override StochasticResult Compute()
    {
        ...
        VectorMath.RollingMax(High, _kPeriod, highBuf);
        VectorMath.RollingMin(Low, _kPeriod, lowBuf);
        ...
        for (int i = 0; i < kLen; i++)
        {
            ...
            k[i] = range > 0 ? (Close[i + _kPeriod - 1] - lowest) / range * 100 : 50;
        }
    ...
    public override void Apply(Axes axes)
    {
        ...
        PlotSignal(axes, result.K, kOffset);
        PlotSignal(axes, result.D, dOffset, $"%D({_dPeriod})", DColor ?? Colors.Tab10Orange).LineStyle = LineStyle.Dashed;
        axes.YAxis.Min = 0;
        axes.YAxis.Max = 100;
    }
}
