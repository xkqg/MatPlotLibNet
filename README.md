...
| Reported | Symptom | Fix on `main` |
|---|---|---|
| 2026-04-19 | **Plot appeared to disappear when interacting with the legend.** Clicking a legend item fired the series-toggle on `pointerdown` (mouse-press) instead of on `click` (full press-and-release). Single- and two-series charts visibly emptied; pressing-and-holding to drag the legend was mechanically impossible because the series hid before any drag could engage. | `SvgLegendToggleScript`: toggle fires on `click` (and `Enter`/`Space`) only — never on `pointerdown`. New regression test `InteractionScriptHarnessTests.LegendItem_PointerdownAlone_DoesNotToggleSeries`; pinned end-to-end via the Phase S Playwright harness (`c:/tmp/legend_repro.py`). |
| 2026-04-19 | **Legend was fixed in place** — no way to reposition a legend without rebuilding the figure server-side. | New `SvgLegendDragScript`: press-and-hold any legend item to drag the entire `<g class="legend">` group; release to drop. Drag detection mirrors Phase R lessons (`isPointerDown` gate so hover does not poison; 5-px² threshold separates click from drag). Coexists with pan/zoom (capture-phase `stopPropagation` on pointerdown prevents pan from racing for `setPointerCapture`; on pointermove during a real drag prevents pan-while-dragging) and with the toggle script (capture-phase one-shot click swallower after a real drag). Translation is client-only (lost on full server re-render — persistence is a small follow-on if needed). 4 new behavioural tests in `LegendDragTests`. |
| 2026-04-19 | **Treemap parent tile click did nothing in the playground.** Two compounding bugs, both latent in v1.7.2 since Phase P (2026-04-18). **(a)** Once the user moved the cursor over the chart, the next click was silently dropped — the `pointermove` listener latched the drag-suppression flag on every hover (no button required), so the click handler's `if (pointerMoved) return;` guard then suppressed the toggle forever. **(b)** Even without prior hover, the click was lost in real browsers because the pan/zoom script's `setPointerCapture(pointerId)` on `pointerdown` redirects the synthetic `click` derived from `pointerup` to the SVG root rather than the rect under the cursor. The treemap script walked up from `e.target` only — finding no `data-treemap-node` on the SVG root, it returned null and the toggle never fired. Both bugs invisible to xUnit (Jint harness has no `setPointerCapture` semantics) but reproduced end-to-end with a real-browser Playwright pixel-compare harness. | `SvgTreemapDrilldownScript`: **(a)** gate the move-threshold check on an explicit `isPointerDown` flag set in `pointerdown` and cleared in `pointerup`/`pointercancel`. **(b)** Two-stage click target resolution — walk up from `e.target` first, then fall back to `document.elementFromPoint(clientX, clientY)` to recover the real rect when capture redirected the click. Two new regression tests pin both layers (`HoverWithoutButtonDown_DoesNotPoisonClickHandler`, `Click_RedirectedToSvgRoot_FallsBackTo_ElementFromPoint`). `DomDocument` gains a stub-injectable `elementFromPoint` so the redirect-redirection scenario is testable in Jint without a real layout engine. `InteractionScriptHarness` `<remarks>` now documents what the harness does NOT simulate (no event bubbling, no capture phase, no `setPointerCapture` target redirection, no built-in `elementFromPoint`) so future bug authors can pick a test path that actually catches their bug class. |
