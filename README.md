...
> **v1.3.0 — Cross-platform native UI controls (Avalonia 12 + Uno Platform).** `MplChartControl` (Avalonia) and `MplChartElement` (Uno) render charts natively via SkiaSharp — no browser, no WebView, no SignalR required. Set `IsInteractive="True"` for local pan / zoom / reset / brush-select using the new managed interaction layer in core (`InteractionController`, six `IInteractionModifier` implementations, `ChartLayout` coordinate transform). The same six-event hierarchy from v1.2.0 (four mutations + two notifications) is reused unchanged — desktop and SignalR share one vocabulary. **3 842 tests green** across 9 test projects (+78 new). Two new NuGet packages: `MatPlotLibNet.Avalonia` and `MatPlotLibNet.Uno`.
>
