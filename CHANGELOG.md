...
- **New behavioural test files** — `ThreeDInteractionIsolationTests.cs` (4 tests, isolating 2D/3D event handlers), `ThreeDRotationParityTests.cs` (10 tests, stacked Theory over the matplotlib drag formula + 360° round-trip + accumulation + past-pole), extended `TwoDZoomCompletenessTests.cs` (5 new theory cases for wheel rate + axis lock). All driven through the existing Phase-1 `InteractionScriptHarness` (Jint engine + XDocument-backed DOM stub) — DRY across the suite.
