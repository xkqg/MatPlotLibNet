...
public class GraphQLSubscriptionIntegrationTests
{
    ...
    public async Task SvgSubscription_DeliversPayloadToSubscriber()
    {
        ...
        await using var stream = await receiver.SubscribeAsync<ChartEventMessage>("ChartSvg:chart-1");

        // Publish on a background task so ReadEventsAsync doesn't hang before send.
        _ = Task.Run(async () =>
        {
            await Task.Delay(50);
            await sender.SendSvgAsync("chart-1", "<svg><rect/></svg>");
        });
    ...
    public async Task JsonSubscription_DeliversPayloadToSubscriber()
    {
        ...
        await using var stream = await receiver.SubscribeAsync<ChartEventMessage>("ChartJson:chart-2");

        _ = Task.Run(async () =>
        {
            await Task.Delay(50);
            await sender.SendJsonAsync("chart-2", "{\"x\":1}");
        });
    ...
    public async Task Subscription_OtherChartId_DoesNotReceive()
    {
        ...
        await using var stream = await receiver.SubscribeAsync<ChartEventMessage>("ChartSvg:chart-A");

        _ = Task.Run(async () =>
        {
            await Task.Delay(50);
            // Send to a DIFFERENT chart's topic.
            await sender.SendSvgAsync("chart-B", "<svg data-leak=\"true\"/>");
        });
